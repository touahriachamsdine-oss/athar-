...
function catInfo(id, lang) {
    const idx = APP_CONFIG.categories.findIndex(c => c.id === id);
    const has = idx >= 0;
...
const DICT = {
    ar: { page: 'استكشف المبادرات', subtitle: 'اكتشف المبادرات والنشاطات المعتمدة', filter_all: 'الكل', empty: 'لا توجد مبادرات حالياً', empty_results: 'لا توجد مبادرات تطابق الاختيار', health: 'صحة', join: 'انضمام' },
    fr: { page: 'Explorer les Initiatives', subtitle: 'Découvrez les initiatives approuvées', filter_all: 'Tout', empty: 'Aucune initiative pour l\'instant', empty_results: 'Aucune initiative ne correspond à ce filtre', health: 'Santé', join: 'Rejoindre' },
    en: { page: 'Explore Initiatives', subtitle: 'Discover approved initiatives and activities', filter_all: 'All', empty: 'No initiatives yet', empty_results: 'No initiatives match this filter', health: 'Health', join: 'Join' }
};

let allData = [];
let activeCat = 'all';

function renderCards(data, lang, emptyMsg) {
...
async function init() {
    ...
    document.getElementById('filters').addEventListener('click', e => {
        ...
        renderCards(search ? filtered.filter(i => (i.title_ar||'').includes(search) || (i.title_fr||'').toLowerCase().includes(search)) : filtered, lang, t.empty_results);
    });
    ...
    document.getElementById('search-input').addEventListener('input', e => {
        ...
        renderCards(q ? base.filter(i => (i.title_ar||'').includes(q) || (i.title_fr||'').toLowerCase().includes(q) || (i.title_en||'').toLowerCase().includes(q)) : base, lang, t.empty_results);
    });
}
init();